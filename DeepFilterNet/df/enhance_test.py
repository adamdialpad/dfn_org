...
def main(args):
    ...
    if args.out_csv:
        ...
        csv_writer.writerow(['filename', 'mean_time_s', 'min_time_s', 'max_time_s', 'std_time_s', 'audio_duration_s', 'mean_rt_factor', 'runs'])
    
    # Optimize model with IPEX for inference if enabled
    model.eval()  # Ensure model is in eval mode
    if args.enable_ipex:
        try:
            logger.info("Optimizing model with Intel Extension for PyTorch...")
            model = ipex.optimize(model, dtype=torch.int8)
            logger.info("IPEX optimization successful")
        except Exception as e:
            logger.warning(f"IPEX optimization failed: {e}, continuing without optimization")
    else:
        logger.info("IPEX optimization disabled")
    ...
    for i, (file, audio, audio_sr) in enumerate(loader):
        ...
        # Run enhancement 25 times to get accurate mean timing
        times = []
        enhanced_audio = None
        ...
        for run in range(25):
            ...
            enhanced_audio = enhance(
                model, df_state, audio.clone(), pad=args.compensate_delay, atten_lim_db=args.atten_lim
            )
            t1 = time.time()
            times.append(t1 - t0)
        ...
        rtf_mean = t_mean / t_audio
        
        # Use the last enhanced audio for saving
        audio = enhanced_audio
        fn = os.path.basename(file)
        p_str = f"{progress:2.0f}% | " if n_samples > 1 else ""
        logger.info(f"{p_str}Enhanced '{fn}' - Mean: {t_mean:.4f}s ±{t_std:.4f}s (RT factor: {rtf_mean:.3f}) over 25 runs")
        
        # Write detailed statistics to CSV if enabled
        if csv_writer:
            csv_writer.writerow([
                fn, 
                f"{t_mean:.6f}", 
...
def run():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output CSV file to save runtime statistics for each audio file.",
    )
    parser.add_argument(
        "--enable-ipex",
        action="store_true",
        help="Enable Intel Extension for PyTorch (IPEX) optimization for better CPU performance.",
    )
