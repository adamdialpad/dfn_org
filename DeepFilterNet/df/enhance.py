...
def main(args):
    ...
    if args.out_csv:
        ...
        csv_writer.writerow(['filename', 'processing_time_s', 'audio_duration_s', 'rt_factor'])
    
    # Optimize model with IPEX for inference (before processing loop)
    model.eval()  # Ensure model is in eval mode before IPEX optimization
    try:
    ...
    for i, (file, audio, audio_sr) in enumerate(loader):
        ...
        if csv_writer:
            csv_writer.writerow([fn, f"{t:.4f}", f"{t_audio:.4f}", f"{rtf:.6f}"])
